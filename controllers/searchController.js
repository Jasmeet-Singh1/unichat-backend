...
exports.searchUsers = async (req, res) => {
  try {
    ...
    const users = await User.find(searchQuery)
      .select(
        'firstName lastName email username role program programType bio coursesEnrolled isVerified isApproved'
      )
    ...
    const sortedUsers = formattedUsers.sort((a, b) => {
      if (q && q.trim()) {
        ...
        const aExactMatch =
          a.fullName.toLowerCase() === queryLower ||
          a.email.toLowerCase() === queryLower ||
          a.username.toLowerCase() === queryLower;
        const bExactMatch =
          b.fullName.toLowerCase() === queryLower ||
...
exports.getUserProfile = async (req, res) => {
  try {
    ...
    if (user.role === 'Mentor') {
      ...
      const alumni = await Alumni.findById(userId).select('gradDate currentJob').lean();
      if (alumni) {
        additionalDetails = {
      ...
      const student = await Student.findById(userId).select('expectedGradDate studentClubs').lean();
      if (student) {
        additionalDetails = {
...
exports.searchMentorsByCourse = async (req, res) => {
  try {
    ...
    const formattedMentors = mentors.map((mentor) => {
      const relevantCourses = mentor.courseExpertise.filter(
        (ce) => ce.course.match(new RegExp(courseCode, 'i')) && acceptableGrades.includes(ce.grade)
      );
